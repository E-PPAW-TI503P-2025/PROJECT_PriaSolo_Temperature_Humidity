...
async function fetchSensorData() {
  try {
    const response = await fetch(
      `${API_BASE_URL}/temperature/history?limit=50`
    );
    if (!response.ok) throw new Error("Network response was not ok");
    const data = await response.json();

    // Convert string timestamps to Date objects and ensure numbers
    return data
      .map((d) => ({
      ...
      .reverse(); // API returns newest first, but chart/array often needs oldest first or consistent order
  } catch (error) {
...
function updateDashboard() {
  ...
  const latestData = sensorData[sensorData.length - 1]; // This logic assumes sensorData is sorted old->new
  // If API returns new->old, this should be sensorData[0]
  // In fetchSensorData, I did .reverse() so it should be old->new (ascending timestamp)

  // Wait, let's double check fetchSensorData:
  // return data.map(...).reverse();
  // API returns "orderBy: { timestamp: 'desc' }" (newest first).
  // So .reverse() makes it oldest first.
  // So sensorData[length-1] is the NEWEST. Correct.
